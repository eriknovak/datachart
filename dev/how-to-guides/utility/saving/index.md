...
Every chart function returns a matplotlib figure. The [datachart.utils.save_figure](https://eriknovak.github.io/datachart/dev/references/utils/#datachart.utils.save_figure) function writes it to disk in the format given by its `format` argument or, when omitted, by the file extension. The theme is baked into the figure when it is created, so saving never consults the global `config`.
...
The [FIG_FORMAT](https://eriknovak.github.io/datachart/dev/references/constants/#datachart.constants.FIG_FORMAT) constant lists every supported format.
