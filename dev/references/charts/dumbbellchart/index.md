...
Two values per category, a dot at each and a connector between them. The [Dumbbell Chart guide](https://eriknovak.github.io/datachart/dev/how-to-guides/charts/dumbbellchart/index.md) shows every feature on real data; this page is the contract: the function, the shape of its data, the keys `style` takes, and the constant each parameter accepts.
...
| `scaley`          | The scale of the value axis ("linear", "log", "symlog", "logit"), whichever way it runs, as on the box plot. See AXIS_SCALE. **TYPE:** \`AXIS_SCALE                                                                                                                                                                                         |
