...
- **A swarm and a raincloud label their range.** A swarm prints each group's
  minimum, median, and maximum through the collision placer, on the points
  holding them (the median on the point nearest it). Labelling every point,
  even stepped, printed numbers at arbitrary ranks that told a reader nothing,
  while the extremes and the centre are what a swarm is read for. So a swarm
  takes no `value_step`. A raincloud prints the same three numbers split by
  part: its box prints the median beside the median line, and its rain prints
  the minimum and maximum on their points; the cloud never labels.
