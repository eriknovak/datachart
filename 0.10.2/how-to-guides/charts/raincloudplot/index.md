...
Long group names and a value axis that reads left to right both call for horizontal rainclouds. `orientation=ORIENTATION.HORIZONTAL` ([ORIENTATION](https://eriknovak.github.io/datachart/0.10.2/references/constants/#datachart.constants.ORIENTATION)) puts the groups on the y-axis, with the cloud above and the box and rain below it; the axis labels and the grid swap with the orientation, and the first group sits at the bottom.
