...
Rank over time, one line per series. The [Bump Chart guide](https://eriknovak.github.io/datachart/0.10.2/how-to-guides/charts/bumpchart/index.md) shows every feature on real data; this page is the contract: the function, the shape of its data, the keys `style` takes, and the constant each parameter accepts.
...
Each record in `data` is a [`LineDataPointAttrs`](https://eriknovak.github.io/datachart/0.10.2/references/charts/linechart/#datachart.typings.LineDataPointAttrs); the `x` and `y` parameters rename its keys.
