...
This section showcases the themes found in the [datachart.themes](https://eriknovak.github.io/datachart/0.9.1/references/themes/index.md) module and how to customize them. Seven predefined themes are available: `DEFAULT`, `GREYSCALE`, `MINIMAL`, `MATERIAL`, `INK`, `HATCH`, and `SKETCH` — each named for its visual trait — see the [Theme Gallery](https://eriknovak.github.io/datachart/0.9.1/how-to-guides/styling/theme-gallery/index.md) for every theme rendered across the full range of chart types.

Themes may also carry defaults for chart settings ([ThemeDefaultAttrs](https://eriknovak.github.io/datachart/0.9.1/references/typings/#datachart.typings.ThemeDefaultAttrs)): `chart_default_show_grid` supplies the grid when a chart call leaves `show_grid` unset (every predefined theme but `SKETCH` ships a muted `"y"` grid), `chart_default_show_values` does the same for bar value labels (on in `MINIMAL`, `MATERIAL`, and `HATCH`), and `plot_hatch_cycle` assigns hatch patterns per bar/histogram series (only `HATCH` ships one). An explicit chart setting always wins over the theme default.

`SKETCH` is the hand-drawn look: its path wobble and white halo are sketch attributes ([SketchStyleAttrs](https://eriknovak.github.io/datachart/0.9.1/references/typings/#datachart.typings.SketchStyleAttrs)) that the panel applies at render time, so matplotlib's global settings never change. It ships the Comic Neue font, so the look is the same on every machine.
...
The [datachart.constants.THEME](https://eriknovak.github.io/datachart/0.9.1/references/constants/#datachart.constants.THEME) module contains all the predefined themes.
...
See the [Theme Gallery](https://eriknovak.github.io/datachart/0.9.1/how-to-guides/styling/theme-gallery/index.md) for every predefined theme rendered across the full range of chart types.
...
The user can create their own theme by defining a new dictionary that has the same structure as the [StyleAttrs](https://eriknovak.github.io/datachart/0.9.1/references/typings/#datachart.typings.StyleAttrs) type.
