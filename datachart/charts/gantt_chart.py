...
def GanttChart(
    ...
    *,
    ...
    Args:
        data: The task records of the schedule: a list of
            `{task, start, end}` dicts with optional `group`, `progress`,
            `depends_on`, and `emphasis` keys. `start` and `end` are `date`,
            `datetime`, `numpy.datetime64`, or pandas `Timestamp` objects;
            date strings are never parsed. A list of such lists draws one
            schedule per subplot. See `GanttTaskAttrs`.
        ...
        sort: The order of the task rows: None (input order), "ascending",
            ...
            order. See `SORT`.
        sort_by: The key `sort` orders by: `"start"` (default) orders every
            row by its start; `"group"` clusters the rows by group, the groups
            ordered by their earliest start and the tasks within a group by
            start. Requires `sort`. See `GANTT_SORT_KEY`.
        emphasis: The emphasis role of the whole schedule ("background" or
            "highlight"), or one role per schedule. See `EMPHASIS`.
        emphasis_rule: A one-key dict that highlights the tasks matching it
            and mutes the rest: `{"above": v}` or `{"below": v}` (strict),
            `{"between": (lo, hi)}` (inclusive), `{"top": n}` or
            `{"bottom": n}`. Reads each task's duration in days; a task's own
            `emphasis` key wins over the rule. See `EmphasisRuleAttrs`.
        style: Style configuration(s) for the schedule. See `GanttStyleAttrs`.
        xtickrotate: Rotation angle for the date-axis tick labels.
        ytickrotate: Rotation angle for the task-axis tick labels.
        xticks_format: The date-axis tick label format: a `DATE_FORMAT`
            member or `strftime` pattern.
