...
def get_subplot_config(
    split: bool, n_charts: int = 1, max_cols: int = 1
) -> Dict[str, int]:
    """Calculate the configuration for subplots in a figure.

    Args:
        split: Whether the charts draw one subplot each.
...
def get_basemap_style(chart_style: dict) -> dict:
    ...
    Returns:
        ...
        `highlight` face and its outline's `edge_color` and `edge_width`.
    ...
    config_attrs = {
        ...
        "countries": [
            ...
            ("edge_color", "plot_basemap_highlight_edge_color"),
