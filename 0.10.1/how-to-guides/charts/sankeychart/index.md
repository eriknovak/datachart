...
A Sankey owns its drawing: its 0 to 1 layout is not a coordinate space another chart can share, so [datachart.utils.Panel](https://eriknovak.github.io/datachart/0.10.1/references/utils/#datachart.utils.Panel) rejects a Sankey figure. [datachart.utils.Grid](https://eriknovak.github.io/datachart/0.10.1/references/utils/#datachart.utils.Grid) arranges it next to other charts, each in its own cell, and a `layout_spec` lets a cell span more than one row or column; the [Grid](https://eriknovak.github.io/datachart/0.10.1/how-to-guides/utility/grid/index.md) guide covers the layout options. The Sankey shows where the energy goes, but not how efficient each sector is, so a [BarChart](https://eriknovak.github.io/datachart/0.10.1/references/charts/barchart/#datachart.charts.BarChart) below it prints the share of each sector's intake that becomes useful work, computed from the same links: transport, the largest sector, wastes the most.
